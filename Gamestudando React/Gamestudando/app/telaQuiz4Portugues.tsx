...
export default function Index() {
  ...
  const perguntaAtualObj = perguntasPortugues[perguntaAtual];
  const respostaCorreta = perguntaAtualObj.correta;



  //Barra de progresso
  const progresso = (perguntaAtual + 1) / perguntasPortugues.length;
  ...
  const confirmarResposta = () => {
    ...
    if (!respostaConfirmada) {
      ...
      if (perguntaAtual < perguntasPortugues.length - 1) {
  ...
  return (
    ...
    >
      ...
      <Text style={{ marginTop: 5 }}>
        {perguntaAtual + 1} / {perguntasPortugues.length}
      </Text>
